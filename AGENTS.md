...
Offline mobile expense tracker built with NativePHP Mobile (Laravel). Full spec lives in `PLAN.md`.

## Status

Features are **not implemented yet**. This is a stock Laravel 13 + NativePHP Mobile scaffold: `routes/web.php` only renders `welcome`, `app/Models` has only `User`, migrations are the Laravel defaults. Build the expense tracker per `PLAN.md` — do not assume any domain models, routes, or views exist.

## Stack

- PHP 8.3+ (8.4.25 installed), Laravel 13.17, NativePHP Mobile 4.3.2
- SQLite (`database/database.sqlite`), Tailwind CSS v4 via `@tailwindcss/vite`, Vite 8
- Livewire + Alpine.js are named in `PLAN.md` but **not installed** yet — add them if used
- Tests are Pest (pestphp/pest + pest-plugin-laravel); PHPUnit is present only as Pest's transitive dependency
- Laravel Boost is installed — MCP server at `php artisan boost:mcp` (configured in `opencode.json`/`.mcp.json`)

## Commands

- `composer test` — clears config cache then runs `php artisan test` (in-memory SQLite)
- `php artisan test --filter=TestName` — run a single test (or `./vendor/bin/pest --filter=TestName`)
- `./vendor/bin/pint` — format PHP (Laravel Pint)
- `npm run build` / `npm run dev` — Vite/Tailwind asset build
- `composer setup` — full bootstrap (install deps, `key:generate`, migrate, npm build)

## NativePHP mobile dev loop

- `composer dev` → `php artisan dev` — starts all dev processes
- `php artisan native:jump` — starts the dev server (HTTP :3000, WS :8081) and prints a QR to load the app on a phone
- `php native <cmd>` is an alias for `php artisan native:<cmd>`
- `php artisan native:install` — installs native resources (writes Android themes to `nativephp/android`); rerun after changing native config
- `php artisan native:run` — build + package + run the app
- Android Gradle project lives in `nativephp/android/` (generated, gitignored)

## Conventions (from PLAN.md)

- Fully offline; no auth, no external APIs/cloud, no paid packages
- Currency is Bangladeshi Taka (৳); centralize formatting — do not hardcode the symbol
- Theme/accent color via CSS variables (`--primary-color` etc.); never hardcode the primary color
- Reusable Blade components (AppLayout, GlassCard, BalanceCard, TransactionItem, ...); avoid duplicating UI
- Data in SQLite (transactions, categories, settings) with foreign keys and validation

## Gotchas

- `.npmrc` sets `ignore-scripts=true`, so use `npm install --ignore-scripts` (or `composer setup`)
- `.env` is gitignored and already sets `DB_CONNECTION=sqlite` and `NATIVEPHP_APP_ID=com.alemamdev.expensetracker`
